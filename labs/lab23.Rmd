---
title: "Lab 23: Chicago Crime Data I"
...
##############################################################################
# 03. It is difficult to do much of anything directly with the raw data. We
...
##############################################################################
# 04. Now, take the mean of the theft variable over the entire dataset:
...
##############################################################################
# 05. Take the sum of the theft variable:
...
##############################################################################
# 06. Take the dataset `ca` and calculate the sum of the variable
# `num_households`:
...
##############################################################################
# 07. Use the filter function to construct a dataset `temp` consisting only of
...
##############################################################################
# 08. Take the mean of the variable arrests on the data `temp`.
...
##############################################################################
# 09. Take the mean of the variable `theft` on the data temp.
...
##############################################################################
# 10. Manually look up the number of households in area 23, Humboldt Park, by
...
# Is this smaller, larger, or about the same as the same measurment over the
# over the entire dataset? Can we safely compare these measurments? If so
# describe the relationship in words.
...
##############################################################################
# 11. You have the tools to get the number of households in Humboldt Park
...
##############################################################################
# 16. Modify the previous plot so that the points are labeled with the area
# name:


##############################################################################
# 17. What is the name of the outlier?

# Answer:

##############################################################################
# 18. (Critical Thinking) Look up the neighborhood that is the outlier on
